...
for i = 1:nApp
    ...
    plotD(i).Davg = cellfun(@(x,y)(sum(sqrt(diff(x).^2 + ...
                                            diff(y).^2))),Px{i},Py{i});
                                        
    % sets the fly indices
    plotD(i).iFly = cellfun(@(x)(sprintf('Fly #%i',x)),...
                        num2cell(1:size(plotD(i).X,2)),'un',0);                                             
    plotD(i).iFrm = cellfun(@(x)(sprintf('Frame #%i',x)),...
                        num2cell(1:size(plotD(i).X,1)),'un',0);                                             
end

...
cP(1) = setParaFields(a{1},'List',{1+isLong,pList},'nGrp','Number of Daily Time Groups',[],{0,~isLong});

% sets the tool-tip strings
if (isLong)
    cP(1).TTstr = 'The number of groups that the day is split up into';
else
    cP(1).TTstr = 'Can''t access this field as experiment(s) are less than 12 hours in duration';
end

...
switch dcObj.pType
    case 'Sleep Duration'
        % case is sleep duration rate
        dcObj.yUnits = 'Min/Hour';
        
    case 'Mean Bout Duration'
        % case is the mean bout duration
        dcObj.yUnits = 'Min/Bout';
        
    otherwise
        % case is the other metrics
        dcObj.yUnits = 'Bouts/Hour';
...
if nGrp == 1
    ...
    for j = 1:nApp
        ...
        if pP.pltDN
            patch(xFillD(ix),yFill(iy),'y',...
                'FaceAlpha',fAlpha,'tag','hDN','HitTest','off')
            patch(xFillN(ix),yFill(iy),'k',...
                'FaceAlpha',fAlpha,'tag','hDN','HitTest','off')
        end
        ...
        if isComb
            % plots the double axis bar
            if pP.plotErr
                [hAxB{j},hBar{j},hErr{j}] = plotDoubleAxisBar...
                    (hAxB{j},xi,sMlt*p(j).N_mn,p(j).Ts_mn,pP.pW,[],...
                    nTick,[NaN 60],sMlt*p(j).N_sem,p(j).Ts_sem);
            else
                [hAxB{j},hBar{j},hErr{j}] = plotDoubleAxisBar...
                    (hAxB{j},xi,sMlt*p(j).N_mn,p(j).Ts_mn,pP.pW,[],...
                    nTick,[NaN 60],[],[]);
